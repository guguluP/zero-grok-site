...
// Keep rest of original landing script behavior
(function () {
  ...
  // Simple can liquid demo on landing if present
  const demo = document.getElementById('demo-liquid');
  if (demo) {
    let level = 72;
    setInterval(() => {
      level = Math.max(8, Math.min(95, level + (Math.random() * 10 - 5)));
      demo.style.height = level + '%';
    }, 2200);
  }
})();
