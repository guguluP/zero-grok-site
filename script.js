...
installButtons.forEach(btn => {
  ...
  btn.setAttribute('download', 'zero-grok-site-main.zip');
});
...
// Plays the "crack open" sound + fizz burst only when the person actually
// interacts with the can itself (click/tap or Enter/Space while it's
// focused) — not on any arbitrary first click on the page.
function popCan() {
  if (!heroCan) return;
...
// Unlock the WebAudio context on the can's own pointerdown so the very
// first tap on the can can play sound immediately (mobile browsers
// require a user gesture on/near the thing that triggers audio).
...
heroCan?.addEventListener('keydown', (e) => {
  if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); popCan(); }
});
